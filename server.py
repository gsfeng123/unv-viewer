...
def init_db():
    ...
    # 检查rsp_ent_name列是否存在，不存在则添加
    try:
        cursor.execute('SELECT rsp_ent_name FROM datasets LIMIT 1')
    except:
        cursor.execute('ALTER TABLE datasets ADD COLUMN rsp_ent_name TEXT')
...
def import_file_to_db(file_path):
    ...
    try:
        ...
        for ds in datasets:
            ...
            rsp_ent_name = ''
            if ds_type == 58:
                rsp_ent_name = str(ds.get('rsp_ent_name', '')).strip()
            
            cursor.execute('''
                INSERT INTO datasets (file_id, dataset_number, dataset_name, dataset_type, data_points, num_pts, id1, id2, id3, id4, rsp_ent_name)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ''', (
                file_id,
                ds_type,
...
def get_db_files():
    ...
    for row in cursor.fetchall():
        files.append({
            'id': convert_db_value(row['id']),
            'file_path': convert_db_value(row['file_path']),
            'file_name': convert_db_value(row['file_name']),
            'file_size': convert_db_value(row['file_size']),
            'file_size_formatted': format_file_size(convert_db_value(row['file_size'])),
            'import_time': convert_db_value(row['import_time']),
            'total_datasets': convert_db_value(row['total_datasets']),
            'total_points': convert_db_value(row['total_points'])
        })
    
    conn.close()
    return jsonify({'files': files})

def convert_db_value(val):
    """转换数据库值为Python原生类型"""
...
def get_db_file(file_id):
    ...
    file_info = {
        'id': convert_db_value(file_row['id']),
        'file_path': convert_db_value(file_row['file_path']),
        'file_name': convert_db_value(file_row['file_name']),
        'file_size': convert_db_value(file_row['file_size']),
        'file_size_formatted': format_file_size(convert_db_value(file_row['file_size'])),
        'import_time': convert_db_value(file_row['import_time']),
        'total_datasets': convert_db_value(file_row['total_datasets']),
        'total_points': convert_db_value(file_row['total_points'])
    }
    
    # 获取数据集列表
    cursor.execute('''
        SELECT id, dataset_number, dataset_name, data_points, num_pts, id1, id2, id3, id4, rsp_ent_name
        FROM datasets WHERE file_id = ? ORDER BY dataset_number
    ''', (file_id,))
    
    datasets = []
    for row in cursor.fetchall():
        ds_num = convert_db_value(row['dataset_number'])
        ds_name = convert_db_value(row['dataset_name'])
        
        # 对于数据集58，使用rsp_ent_name作为名称
        if ds_num == 58:
            rsp_name = convert_db_value(row['rsp_ent_name'])
            if rsp_name and rsp_name != '0' and rsp_name.strip():
                ds_name = rsp_name.strip()
        
        datasets.append({
            'id': convert_db_value(row['id']),
            'dataset_number': ds_num,
            'dataset_name': ds_name,
            'data_points': convert_db_value(row['data_points']),
            'num_pts': convert_db_value(row['num_pts']),
            'id1': convert_db_value(row['id1']),
            'id2': convert_db_value(row['id2']),
            'id3': convert_db_value(row['id3']),
            'id4': convert_db_value(row['id4']),
            'rsp_ent_name': convert_db_value(row['rsp_ent_name'])
        })
